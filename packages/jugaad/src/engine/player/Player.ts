...
import { Board, ControlMove, Move, NormalMove, Square, WeakMove } from '../board'
...
export default class Player {
  ...
  calculateLegalMoves() {
    ...
    for (const piece of this.pieces) {
      ...
      const legals = currentSquare.isForbiddenZone && !piece.isGodman ? [] :
        piece.calculateLegalMoves(currentSquare, toRetreat)
      // this.legalMoves.push(...legals);
      this.moves.set(currentSquare, legals)
    }
    ...
    for (const officer of this.rajrishi.adjacentOpponentOfficers) {
      ...
      for (const { destinationSquare } of legals) {
        ...
        temp.push(new ControlMove(officerSquare, destinationSquare))
      }
      this.moves.set(officerSquare, temp)
    }
  }
  private initPieces() {
    for (const piece of this.pieces) {

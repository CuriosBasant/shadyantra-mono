...
export default abstract class RoyalPiece extends Piece {
  ...
  helpNearbyRoyals(board: Board) {
    ...
    if (currentSquare.isRoyalNearby) return
...
export class Guptchar extends RoyalPiece {
  ...
  calculateLegalMoves(currentSquare: Square) {
    const moves = super.calculateLegalMoves(currentSquare),
      lastRank = { w: 8, b: 1 }
    if (currentSquare.rank == lastRank[this.alliance.symbol]) {
      currentSquare.createPromotionMove(moves, currentSquare)
    }
    return moves
  }
}